...
if __name__ == "__main__":
    ...
    def parse_history(history):
        if isinstance(history, str):
            return [int(item.split()[0][1:]) for item in history.split()]
        return []


    behaviors_data["History"] = behaviors_data["History"].apply(parse_history)


    # 将Impressions中的点击行为拆分成列表
    def parse_impressions(impressions):
        return [int(item.split("-")[1]) for item in impressions.split()]


    behaviors_data["Impressions"] = behaviors_data["Impressions"].apply(parse_impressions)

    # 训练集和验证集划分
    train_data = behaviors_data[behaviors_data["Time"].str.contains("11/15/2019")]
    val_data = behaviors_data[behaviors_data["Time"].str.contains("11/22/2019")]

    X_train = {"user_id": np.array(train_data["User_ID"]),
               "hist_news_id": np.array([hist for hist in train_data["History"]]),
               "impressions": np.array([imps for imps in train_data["Impressions"]])}

    y_train = np.array([1 if click == 1 else 0 for clicks in train_data["Impressions"]])

    user_ids = behaviors_data["User_ID"].unique()
    user_id_mapping = {uid: idx for idx, uid in enumerate(user_ids)}
    user_id_max = len(user_ids)

    # 特征定义
    feature_columns = [SparseFeat('user_id', user_id_max, embedding_dim=8),
                       VarLenSparseFeat(
                           SparseFeat('hist_news_id', vocabulary_size=max(news_data["News_ID"]) + 1, embedding_dim=8),
                           maxlen=50)]

    behavior_feature_list = ["hist_news_id"]
